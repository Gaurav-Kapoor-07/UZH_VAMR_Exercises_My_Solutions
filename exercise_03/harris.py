...
def harris(img, patch_size, kappa):
    ...
    # I_x = signal.convolve2d(in1=sobel_x, in2=img, mode='valid').astype(float)

    # I_y = signal.convolve2d(in1=sobel_y, in2=img, mode='valid').astype(float)
    ...
    lambda_1 = np.pad(lambda_1, [(pr+1, pr+1), (pr+1, pr+1)], mode='constant', constant_values=0)
